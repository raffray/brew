...
//  if(trackName == NULL)  return SUCCESS; // FAILURE ?
  ...
  if     (albumArtist!=NULL)  buffer_append_(&buf, albumArtist, true);
  else
  {    if(artist     ==NULL)  buffer_append (&buf, "Unknown Artist", true);
    else                      buffer_append_(&buf, artist, true);
  }

  mkdir((char*)buf.data, S_IRWXU); // <=== should make sure we are within the working directory; if artist == "../something" ... :)

  if(album!=NULL)   buffer_append_(&buf, album, true);
  else              buffer_append (&buf, "Unknown Album", true);

  mkdir((char*)buf.data, S_IRWXU); // <=== should make sure we are within the working directory; if artist == "../something" ... :)

  if(trackNb!=NULL)
    { trackNum    = atoi((char*)trackNb);
  ...
  if((artist!=NULL) && (albumArtist!=NULL) && strcmp_(artist, albumArtist))
    { buffer_append_(&buf, artist, false);
      buffer_append (&buf, " - " , false); }

  if(trackName!=NULL)   buffer_append_(&buf, trackName, false);
  ...
  if(filename_check(&buf, ".mp3") == SUCCESS)
    {
      rename(filename, (char*)buf.data); //resetFile(buf.data);
      fprintf(ostream, "%s --> %s\n", filename, buf.data);
    }
  else
