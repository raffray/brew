...
{ U4 bytes;
  ...
  buffer_init_setSize(&newBuf, buf->usedSize);  // if no byte-sequence is found for resync, the new data will be identical. Otherwise, it will be shortened
...
/*
UC resyncBuf(char *str, U4 *size)  // same as resyncBuf but doesn't use a new buffer; it can be done "on the spot"
{
  buffer_t buf;
  U4 finalSize = *size;
  U4  readPos = 0;
  ...
  buffer_open(&buf, str, *size); //printf("=== %d ==\n", *size);
  while(readPos<*size)
